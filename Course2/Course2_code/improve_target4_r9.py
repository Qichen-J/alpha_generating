#!/usr/bin/env python3
"""
Target4 crowding-reversal R9
=================================
R8 key findings:
  - Truncation has ZERO effect (all give identical results) → not the bottleneck
  - adv20/relvol signals worse than raw dolvol → dolvol is optimal
  - d63: Sha=1.25✅ Fit=0.96 Ret/TO=0.589  ← new record
  - d70: Sha=1.23❌ Fit=0.96 Ret/TO=0.613  ← Sha drops here

Sha drops somewhere between d63 and d70. Fine-grained sweep needed.
Also: Sha=1.25 is the floor, not a buffer. Need to push Sha UP.

R9 strategy:
  Group A: d64/d65/d66/d67/d68 — find exact Sha drop-off point
           If any hits Sha≥1.25 with Ret/TO≥0.64 → ALL PASS
  Group B: dolvol z=252 + subind + d60/d65 — longer zscore window stabilizes signal → higher Sha?
           At d10, z252 gave Sha=1.50 vs z126 Sha=1.53 (similar). Worth testing at heavy decay.
  Group C: volume lookback v10 (ts_delta(volume*close, 10)) + subind + d60/d65
           Slower volume signal → less noise → higher Sha potential
  Group D: mixed — use ts_rank instead of rank for stationarity, d63 base
"""
import json
import os
import time
...
DOLVOL_V10 = "rank(ts_zscore(ts_delta(volume * close, 10), 126)) * -rank(ts_delta(close, 3))"
DOLVOL_V10_Z252 = "rank(ts_zscore(ts_delta(volume * close, 10), 252)) * -rank(ts_delta(close, 3))"
# ts_rank variant — rolling rank for stationarity
DOLVOL_TSRANK = "ts_rank(ts_zscore(ts_delta(volume * close, 5), 126), 63) * -rank(ts_delta(close, 3))"

VARIANTS = [
    # Group A: fine-grain decay sweep d64–d68
    (
        "T4_dolvol_subind_d64",
        DOLVOL,
        {**SUBIND, "decay": 64},
    ),
    (
        "T4_dolvol_subind_d65",
        DOLVOL,
        {**SUBIND, "decay": 65},
    ),
    (
        "T4_dolvol_subind_d66",
        DOLVOL,
        {**SUBIND, "decay": 66},
    ),
    (
        "T4_dolvol_subind_d67",
        DOLVOL,
        {**SUBIND, "decay": 67},
    ),
    (
        "T4_dolvol_subind_d68",
        DOLVOL,
        {**SUBIND, "decay": 68},
    ),
    # Group B: z252 at d60/d65 — stabilize signal with longer normalization window
    (
    ...
    (
        "T4_dolvol_z252_subind_d65",
        DOLVOL_Z252,
        {**SUBIND, "decay": 65},
    ),
    # Group C: v10 (slower volume signal) at d60/d65
    (
        "T4_dolvol_v10_subind_d60",
        DOLVOL_V10,
        {**SUBIND, "decay": 60},
    ),
    (
        "T4_dolvol_v10_subind_d65",
        DOLVOL_V10,
        {**SUBIND, "decay": 65},
    ),
    # Group D: v10+z252 at d60 — combined stabilization
    (
        "T4_dolvol_v10z252_subind_d60",
        DOLVOL_V10_Z252,
        {**SUBIND, "decay": 60},
    ),
]
...
def main():
    ...
    for idx, (name, expr, settings) in enumerate(VARIANTS, 1):
        ...
        print(f"  decay={settings.get('decay', 0)} neut={settings.get('neutralization')}")
    ...
    out_path = os.path.join(OUTDIR, f"target4_r9_{timestamp}.json")
