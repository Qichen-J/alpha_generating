...
BASE_RAW = "rank(ts_zscore(ts_delta(volume, 3), 126)) * -rank(ts_delta(close, 5))"

VARIANTS = [
    ("T4_base_d4", BASE_RAW, {**BASE_SETTINGS, "decay": 4}),
    ("T4_base_d6", BASE_RAW, {**BASE_SETTINGS, "decay": 6}),
    ("T4_base_d8", BASE_RAW, {**BASE_SETTINGS, "decay": 8}),
    (
        "T4_v5_p5_d6",
        "rank(ts_zscore(ts_delta(volume, 5), 126)) * -rank(ts_delta(close, 5))",
        {**BASE_SETTINGS, "decay": 6},
    ),
    (
        "T4_v3_p3_d6",
        "rank(ts_zscore(ts_delta(volume, 3), 126)) * -rank(ts_delta(close, 3))",
        {**BASE_SETTINGS, "decay": 6},
    ),
    (
        "T4_v3_p10_d6",
        "rank(ts_zscore(ts_delta(volume, 3), 126)) * -rank(ts_delta(close, 10))",
        {**BASE_SETTINGS, "decay": 6},
    ),
    (
        "T4_volrank252_p5_d6",
        "rank(ts_rank(ts_delta(volume, 3), 252)) * -rank(ts_delta(close, 5))",
        {**BASE_SETTINGS, "decay": 6},
    ),
    (
        "T4_cap_bucket_d6",
        "g=bucket(rank(cap), range='0.1,1,0.1');raw=" + BASE_RAW + ";group_neutralize(raw,g)",
        {**BASE_SETTINGS, "decay": 6},
    ),
    (
        "T4_smooth5_d6",
        "raw=" + BASE_RAW + ";ts_decay_linear(raw,5)",
        {**BASE_SETTINGS, "decay": 6},
    ),
    (
        "T4_trade_adv20_d6",
        "trade_when(volume > adv20, " + BASE_RAW + ", -1)",
        {**BASE_SETTINGS, "decay": 6},
    ),
    (
        "T4_trade_absret07_d6",
        "trade_when(abs(returns) < 0.07, " + BASE_RAW + ", -1)",
        {**BASE_SETTINGS, "decay": 6},
    ),
]
...
def main() -> None:
    ...
    output_path = os.path.join(OUTDIR, f"target4_improve_{stamp}.json")
