...
VARIANTS = [
    ("TT3_base_d20", f"group_neutralize({RAW}, industry)", {**BASE_SETTINGS, "decay": 20}),
    ("TT3_base_d25", f"group_neutralize({RAW}, industry)", {**BASE_SETTINGS, "decay": 25}),
    ("TT3_base_d30", f"group_neutralize({RAW}, industry)", {**BASE_SETTINGS, "decay": 30}),
    ("TT3_smooth15_d20", f"raw={RAW};group_neutralize(ts_decay_linear(raw,15),industry)", {**BASE_SETTINGS, "decay": 20}),
    ("TT3_smooth20_d20", f"raw={RAW};group_neutralize(ts_decay_linear(raw,20),industry)", {**BASE_SETTINGS, "decay": 20}),
    ("TT3_absret003_d15", f"trade_when(abs(returns) < 0.03, group_neutralize({RAW}, industry), -1)", {**BASE_SETTINGS, "decay": 15}),
    ("TT3_absret0025_d15", f"trade_when(abs(returns) < 0.025, group_neutralize({RAW}, industry), -1)", {**BASE_SETTINGS, "decay": 15}),
    ("TT3_combined_gate_d20", f"trade_when((abs(returns) < 0.04) and (volume > adv20), group_neutralize({RAW}, industry), -1)", {**BASE_SETTINGS, "decay": 20}),
    ("TT3_combined_gate_d25", f"trade_when((abs(returns) < 0.03) and (volume > adv20), group_neutralize({RAW}, industry), -1)", {**BASE_SETTINGS, "decay": 25}),
    ("TT3_cap_bucket_smooth_d20", "g=bucket(rank(cap), range='0.1,1,0.1');raw=" + RAW + ";group_neutralize(ts_decay_linear(group_neutralize(raw,g),10),industry)", {**BASE_SETTINGS, "decay": 20}),
]
...
def main() -> None:
    ...
    output_path = os.path.join(OUTDIR, f"term_twist_improve_r3_{stamp}.json")
    with open(output_path, "w", encoding="utf-8") as handle:
        json.dump(results, handle, ensure_ascii=False, indent=2)

    print("\n" + "=" * 80)
    print("Term Twist R3 汇总:")
