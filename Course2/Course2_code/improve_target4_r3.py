#!/usr/bin/env python3
"""
Target4 crowding-reversal R3
=================================
R2 best: T4_v5p3_d10 → Sha=1.41✅ Fit=0.76❌ TO=0.4013 Ret=0.1163
         T4_v5p3_d6  → Sha=1.51✅ Fit=0.73❌ TO=0.5007

Need Fit≥1.0: Sha × sqrt(Ret/TO) ≥ 1.0
With Sha=1.41, Ret=0.116 → need TO ≤ 0.23

R3 goals:
  Group A: v5p3 heavy decay d15/d20/d25 — push TO lower, check Sharpe floor
  Group B: v5p3_d10 with SUBINDUSTRY neutralization — finer grouping → lower TO
  Group C: zscore on price component → smoother signal → lower TO
  Group D: v5p3 with longer zscore lookback 252 — more stable volume signal
  Group E: TOP500 universe — concentrated, different dynamics
"""
import json
import os
import time
...
VARIANTS = [
    # Group A: heavy decay sweep on v5p3
    (
        "T4_v5p3_d15",
        V5P3,
    ...
    # Group B: SUBINDUSTRY neutralization (finer groups → fewer net trades → lower TO)
    (
        "T4_v5p3_d6_subind",
        V5P3,
        {**BASE_SETTINGS, "decay": 6, "neutralization": "SUBINDUSTRY"},
    ),
    (
        "T4_v5p3_d10_subind",
        V5P3,
        {**BASE_SETTINGS, "decay": 10, "neutralization": "SUBINDUSTRY"},
    ),
    # Group C: add zscore on price component → smoother daily changes → lower TO
    (
        "T4_v5_zp3_d10",
        "rank(ts_zscore(ts_delta(volume, 5), 126)) * -rank(ts_zscore(ts_delta(close, 3), 63))",
        {**BASE_SETTINGS, "decay": 10},
    ),
    (
        "T4_v5_zp5_d10",
        "rank(ts_zscore(ts_delta(volume, 5), 126)) * -rank(ts_zscore(ts_delta(close, 5), 63))",
        {**BASE_SETTINGS, "decay": 10},
    ),
    # Group D: longer volume zscore window 252 — more stable, fewer extreme values
    (
        "T4_v5p3_z252_d10",
        "rank(ts_zscore(ts_delta(volume, 5), 252)) * -rank(ts_delta(close, 3))",
        {**BASE_SETTINGS, "decay": 10},
    ),
    # Group E: TOP500 universe — smaller, fundamentally different alpha behavior
    (
        "T4_v5p3_d10_top500",
        V5P3,
        {**BASE_SETTINGS, "decay": 10, "universe": "TOP500"},
    ),
    # Group F: MARKET neutralization — less constrained than INDUSTRY
    (
        "T4_v5p3_d10_market",
        V5P3,
        {**BASE_SETTINGS, "decay": 10, "neutralization": "MARKET"},
    ),
]
...
def main() -> None:
    ...
    output_path = os.path.join(OUTDIR, f"target4_r3_{stamp}.json")
