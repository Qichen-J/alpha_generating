...
SPREAD = "ts_zscore(returns, 20) - ts_zscore(returns, 63)"
MID = "rank(ts_zscore(returns, 63))"
RAW = f"-rank(ts_delta({SPREAD}, 10)) * {MID}"

VARIANTS = [
    ("TT4_base_d30", f"group_neutralize({RAW}, industry)", {**BASE_SETTINGS, "decay": 30}),
    (
        "TT4_delta20_d30",
        f"group_neutralize(-rank(ts_delta({SPREAD}, 20)) * {MID}, industry)",
        {**BASE_SETTINGS, "decay": 30},
    ),
    (
        "TT4_delta30_d30",
        f"group_neutralize(-rank(ts_delta({SPREAD}, 30)) * {MID}, industry)",
        {**BASE_SETTINGS, "decay": 30},
    ),
    (
        "TT4_spread_level_d30",
        f"group_neutralize(-rank({SPREAD}) * {MID}, industry)",
        {**BASE_SETTINGS, "decay": 30},
    ),
    (
        "TT4_spread_mean5_d30",
        f"group_neutralize(-rank(ts_mean({SPREAD}, 5)) * {MID}, industry)",
        {**BASE_SETTINGS, "decay": 30},
    ),
    (
        "TT4_spread_mean10_d30",
        f"group_neutralize(-rank(ts_mean({SPREAD}, 10)) * {MID}, industry)",
        {**BASE_SETTINGS, "decay": 30},
    ),
    (
        "TT4_delta20_midmean5_d30",
        f"group_neutralize(-rank(ts_delta({SPREAD}, 20)) * rank(ts_mean(ts_zscore(returns, 63), 5)), industry)",
        {**BASE_SETTINGS, "decay": 30},
    ),
    (
        "TT4_spread_level_subind_d30",
        f"group_neutralize(-rank({SPREAD}) * {MID}, subindustry)",
        {**BASE_SETTINGS, "decay": 30, "neutralization": "SUBINDUSTRY"},
    ),
    (
        "TT4_spread_level_capind_d30",
        "g=bucket(rank(cap), range='0.1,1,0.1');"
        f"raw=-rank({SPREAD}) * {MID};"
...
def main() -> None:
    ...
    output_path = os.path.join(OUTDIR, f"term_twist_improve_r4_{stamp}.json")
    with open(output_path, "w", encoding="utf-8") as handle:
        json.dump(results, handle, ensure_ascii=False, indent=2)

    print("\n" + "=" * 80)
    print("Term Twist R4 汇总:")
