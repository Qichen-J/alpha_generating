#!/usr/bin/env python3
"""
Target4 crowding-reversal R4
=================================
R3 best: T4_v5p3_d6_subind  → Sha=1.64✅ Fit=0.76❌ TO=0.5059
         T4_v5p3_d10_subind → Sha=1.52✅ Fit=0.78❌ TO=0.4039

Key insight: SUBINDUSTRY neutralization significantly boosts Sharpe (+0.13)
Target: SUBINDUSTRY + heavy decay to cut TO from 0.40 → 0.25

Fit formula: Sha × sqrt(Ret/TO) ≥ 1.0
With Sha=1.5, Ret=0.107: need TO ≤ 0.24

R4 strategy:
  Group A: SUBINDUSTRY + d15/d20/d25 — main attack
  Group B: Normalized volume signal (delta/adv20) — reduce raw size effect
  Group C: SUBINDUSTRY + d10 + higher truncation 0.1
  Group D: volume rank (ts_rank) with SUBINDUSTRY
"""
import json
import os
import time
...
# Normalized: delta volume / adv20 (volume change relative to average) 
V5P3_NORM = "rank(ts_zscore(ts_delta(volume, 5) / adv20, 126)) * -rank(ts_delta(close, 3))"
# ts_rank variant with SUBINDUSTRY
V5P3_TSRANK = "rank(ts_rank(ts_delta(volume, 5), 126)) * -rank(ts_delta(close, 3))"

VARIANTS = [
    # Group A: SUBINDUSTRY + heavy decay
    (
        "T4_v5p3_subind_d15",
        V5P3,
        {**SUBIND, "decay": 15},
    ...
    # Group B: normalized volume with SUBINDUSTRY
    (
        "T4_norm_subind_d6",
        V5P3_NORM,
        {**SUBIND, "decay": 6},
    ...
    # Group C: SUBINDUSTRY + higher truncation 0.1
    (
        "T4_v5p3_subind_d10_trunc10",
        V5P3,
        {**SUBIND, "decay": 10, "truncation": 0.1},
    ),
    (
        "T4_v5p3_subind_d15_trunc10",
        V5P3,
        {**SUBIND, "decay": 15, "truncation": 0.1},
    ),
    # Group D: ts_rank (uniform distribution) with SUBINDUSTRY
    (
        "T4_tsrank_subind_d6",
        V5P3_TSRANK,
        {**SUBIND, "decay": 6},
...
def main() -> None:
    ...
    output_path = os.path.join(OUTDIR, f"target4_r4_{stamp}.json")
